...
import {
  ...
  ehBlobUrl,
} from "@/lib/influenciador/imagem-rascunho";
...
export function PassoDadosBasicos({
  ...
  async function handleFotoChange(file: File | null) {
    if (ehBlobUrl(draft.fotoPerfilUrl)) {
    ...
    try {
      ...
      onChange({ fotoPerfilUrl: dataUrl });
    } catch {
      toast.error(
        "Não foi possível salvar a foto no rascunho. Tente outra imagem.",
      );
      onChange({ fotoPerfilUrl: null });
    } finally {
        ...
        <div className="flex flex-col items-center gap-3 sm:flex-row sm:items-start">
          ...
          <div className="space-y-2 text-center sm:text-left">
            ...
            <input
              ...
              className="sr-only"
              onChange={(e) => {
                void handleFotoChange(e.target.files?.[0] ?? null);
              }}
            />
            ...
            <p className="text-texto-secundario text-xs font-normal">
              A foto fica salva no rascunho se você sair e voltar depois.
            </p>
          </div>
        </div>
          ...
          <p className="text-sm">
            {dadosEssenciaisOk ? (
              <>
                ...
                — perfis completos e verificados recebem prioridade nas
              ...
              <>
                ...
                e verificados recebem prioridade nas sugestões para empresas.
                Dedique alguns minutos para preencher com atenção.
              </>
            )}
          </p>
        </div>
